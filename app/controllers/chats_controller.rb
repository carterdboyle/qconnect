class ChatsController < ApplicationController
  ...
  # GET /v1/chats/:id/messages?after_id=123&limit=200
  ...
  def messages_since
    ...
    msgs = convo.messages.where("id > ?", after_id).order(:id).limit(limit);
  ...
  def last_read
    ...
    render json: { last_read_message_id: cr&.last_read_message_id || 0 }
  rescue ActiveRecord::RecordNotFound
  ...
  def summary
    ...
    convos.includes(:messages).find_each do | c |
      ...
      unread = c.messages.where(recipient_id: current_user.id).where("id > ?", last_read_id).count
